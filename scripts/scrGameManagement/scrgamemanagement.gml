function save_game(position) {
	...
	var data = {
		...
		items: global.items,
		lastboss_phase: global.lastBossPhase,
		fan_activate: global.S3_fanON
	};
...
function load_game(position) {
	...
	global.lastBossPhase = data.lastboss_phase;
	global.S3_fanON = data.fan_activate;
...
function change_volume(soundType) {
	var dir = (is_held(global.controls.right) - is_held(global.controls.left));
	switch soundType 
	{
		case "Master":
			global.display.volMaster += 0.01 * dir;
			global.display.volMaster = clamp(global.display.volMaster, 0, 1);
		break;
		case "BGM":
			global.display.volBGM += 0.01 * dir;
			global.display.volBGM = clamp(global.display.volBGM, 0, 1);
		break;
		case "SFX":
			global.display.volSFX += 0.01 * dir;
			global.display.volSFX = clamp(global.display.volSFX, 0, 1);
		break;
	}
}
